...
import com.hurence.historian.spark.compactor.Compactor;
import com.hurence.historian.spark.compactor.CompactorConf;
import com.hurence.historian.spark.compactor.ConfigLoader;
import com.hurence.historian.spark.compactor.SolrConf;
...
public class CompactorTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(CompactorTest.class);

    static MiniSolrCloudCluster cluster;
    static CloudSolrClient cloudClient;
    static String zkAddressSolr;
    ...
    public static void beforeAll() throws Exception {
        ...
        File solrXml = new File(CompactorTest.class.getClassLoader().getResource("solr-embedded-conf/solr.xml").getPath());
        String solrXmlContents  = TestSolrCloudClusterSupport.readSolrXml(solrXml);

        File targetDir = new File("target");
        if (!targetDir.isDirectory())
            fail("Project 'target' directory not found at :" + targetDir.getAbsolutePath());

        File testWorkingDir = new File(targetDir, "scala-solrcloud-" + System.currentTimeMillis());
        ...
        ServletHolder solrSchemaRestApi  = new ServletHolder("SolrSchemaRestApi", ServerServlet.class);
        solrSchemaRestApi.setInitParameter("org.restlet.application", "org.apache.solr.rest.SolrSchemaRestApi");
        extraServlets.put(solrSchemaRestApi, "/schema/*");

        MiniSolrCloudCluster cluster = new MiniSolrCloudCluster(1, null /* hostContext */,
    ...
    public void beforeEach() {
        clearCollection(cloudClient, SolrITHelper.COLLECTION_HISTORIAN);
    }

    private static void initSolr() throws InterruptedException, SolrServerException, IOException {


        // Create collection with schema
       SolrCloudUtilForTests.buildChunkCollection(SolrITHelper.COLLECTION_HISTORIAN, null, 1, cloudClient);
     ...
     */
    @Test
    public void testCompactor(SparkSession sparkSession) throws InterruptedException {
       ...
       // printSolrJsonDocs(cloudClient, SolrITHelper.COLLECTION_HISTORIAN);
        compactor.doCompact();
      //  System.out.println("After recompaction: ");
      //  printSolrJsonDocs(cloudClient, SolrITHelper.COLLECTION_HISTORIAN);
        //printUnChunkedSolrJsonDocs(cloudClient, SolrITHelper.COLLECTION_HISTORIAN);
