...
public class ImportCsvEndPointIT {
    ...
    public void testCsvFileImportSeveralFiles(Vertx vertx, VertxTestContext testContext) {
        ...
        MultipartForm multipartForm = MultipartForm.create()
                .textFileUpload("my_csv_file", "datapoints.csv", pathCsvFile, "text/csv")
                .textFileUpload("my_csv_file_2", "datapoints2.csv", pathCsvFile2, "text/csv");
        List<RequestResponseConfI<?>> confs = Arrays.asList(
                new MultipartRequestResponseConf<JsonObject>(IMPORT_CSV_ENDPOINT,
                        multipartForm,
    ...
    //TODO Feiz si tu trouves un meilleurs moyen que le nombre de ligne pour controller la taille max du fichier libre a toi.
    // Par exemple la taille du fichier... Par contre il faut un test qui met cela en evidence (parles en moi...)
    @Test
    @Timeout(value = 5, timeUnit = TimeUnit.SECONDS)
