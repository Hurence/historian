...
setup() {
  ...
  solr_url="http://historian-solr-0:${SOLR_PORT:-8983}/solr"
  max_attempts=12
  wait_seconds=5
  ...
  curl --location --request POST 'http://historian-solr-0:8983/solr/admin/configs?action=UPLOAD&name=historian' \
    --header 'Content-Type: application/zip' \
    --data-binary '@/opt/historian/historian-configset.zip'

  echo "step 2 : create the collection"
  curl --location --request POST 'http://historian-solr-0:8983/v2/c' \
