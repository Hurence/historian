...
/**
  ...
  * columns.
  */
final class UnChunkytizer(override val uid: String)
  extends Model[UnChunkytizer] with DefaultParamsWritable {

  def this() = this(Identifiable.randomUID("chunkytizer"))
  ...
  def transform(df: Dataset[_]): DataFrame = {
   ...
   df.withColumn("points", unchunk(col($(chunkCol)), col("start"), col("end")))
      ...
      .select( col("name"),col("point._2").as("value" ), col("point._1").as("timestamp"), col("tags"))
  ...
  override def copy(extra: ParamMap): UnChunkytizer = {
    defaultCopy[UnChunkytizer](extra).setParent(parent)
  }
